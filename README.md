...
- Monthly dashboard `재고,판매,입고 추이` table uses a 4-row structure:
  `재고자산 / 택매출액(월) / YOY(월) / 입고금액`
- Weekly dashboard keeps the detailed row structure:
  `재고자산 / 택매출액(1주) / YOY(1주) / 택매출액(4주|8주|12주) / YOY(4주|8주|12주) / 입고금액`
...
## ACC Total Weeks Rule

- Monthly dashboard `전체ACC` card must use the API-level total calculation (`totalWeeks`, `totalPreviousWeeks`) instead of a weighted average of item-level weeks.
- Weekly dashboard `전체ACC` card must also use `weeklyData.totalWeeks`, `weeklyData.totalPreviousWeeks` so the brand summary card and the detail page show the same number.
- Weekly item-level cards still use the 4-week sales formula:
  `재고주수 = 기말재고 ÷ (최근 4주 택매출액 ÷ 4)`
